...
MODIFIED: Removed Step 4 (matchmaker), as number_detection now
handles the pairing directly.
"""
...
# from matchmaker import pair_numbers_to_dots # LÉPÉS KIVÉVE
...
def run_pipeline(config, image_path, expected_range):
    ...
    try:
        ...
        logging.info("STEP 3: Number Detection (and Pairing)")
        # Ez a szkript most már a párosítást is elvégzi
        run_segment_based_detection(config, image_path.name, expected_range)
        
        # logging.info("STEP 4: Dot-Number Matching") # KIVÉVE
                
        logging.info("PIPELINE BEFEJEZVE.")
